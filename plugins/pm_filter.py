...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    offset = 0
    btn.append([InlineKeyboardButton(text="↭ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ↭", callback_data="close_data")])
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        ...
        pass
    if lang != "homepage":
        search = f"{search} {lang}" if lang != "homepage" else search 
    BUTTONS[key] = search

    files, offset, total_results = await get_search_results(chat_id, search, offset=0, filter=True)
    if not files:
        await query.answer("⚠️ No files found for this filter", show_alert=True)
        return
        return
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("🍿 𝘼𝙡𝙡 𝙁𝙞𝙡𝙚𝙨", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("🌐 𝙇𝙖𝙣𝙜𝙪𝙖𝙜𝙚𝙨", callback_data=f"languages#{key}"),
            InlineKeyboardButton("📆 𝙔𝙚𝙖𝙧𝙨", callback_data=f"years#{key}")
        ])
    else:
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    searchagain = search
    if qual != "homepage":
        search = f"{search} {qual}" if qual != "homepage" else search 
    BUTTONS[key] = search
